...
## フォントイメージ  
![Font Image 12p](/Source/FontImage12pHidemaru.png)
![Font Image 26p](/Source/FontImage26pChrome.png)
...
・MyricaM.ttf ： 等幅フォントです。プログラミング用に作成しました。  
　　　ASCII文字はヒンティングされていますので、Windowsでもそれなりに綺麗に表示されます。  
  
・MyricaP.ttf ： なんちゃってプロポーショナルフォントです。  
　　　グリフはそのままに、文字の左右スペースを削り、プロポーショナル文字っぽく作成しました。  
　　　ただし、数字(0-9)は幅固定していますので、エクセルで数値表示しても、桁がずれません。  
　　　ASCII文字はヒンティングされていますので、Windowsでもそれなりに綺麗に表示されます。  
  
・MyricaN.ttf ： 幅を狭くしたフォントです。一定幅で多くの文字を表示できるように作成しました。  
...
ただし、ASCII文字(0x21-0x7E) 以外については、ヒンティングはありません。  
(ヒンティング付きの日本語フリーフォントがあれば、今後合成するかもしれません)
(自動でヒンティングを付加してくれる ttfautohint という素晴らしいソフトもありますが、個人的には日本語文字が汚くなると感じているため、現時点では採用していません。)
